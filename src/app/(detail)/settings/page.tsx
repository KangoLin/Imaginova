...
export default function SettingsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (newPassword !== confirmPassword) { toast(t("settings.passwordsDoNotMatch"), "error"); return; }
    setSaving(true);
    try {
  ...
  return (
    <main className="max-w-lg mx-auto px-6 pt-24 pb-12 animate-fade-in">
      <div className="mb-8">
        <h1 className="text-xl sm:text-2xl font-bold tracking-tight mb-1">{t("settings.title")}</h1>
        <p className="text-muted-foreground text-sm">{t("settings.subtitle")}</p>
      </div>

      <Card>
        <CardHeader className="pb-3"><CardTitle className="text-sm">{t("settings.changePassword")}</CardTitle></CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" disabled={saving} className="w-full h-10 gap-2">
