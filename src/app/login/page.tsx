    ...
    <main className="min-h-dvh flex items-center justify-center px-4 relative">
      ...
      <Card className="w-full max-w-sm animate-fade-in">
        <CardHeader className="pb-4">
          <CardTitle className="text-center text-lg font-bold">{t("auth.welcomeBack")}</CardTitle>
          ...
          <form action="/api/login?redirect=/dashboard" method="POST" className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <Link href="/forgot-password" className="text-xs text-muted-foreground hover:text-primary hover:underline">{t("auth.forgotPassword")}</Link>
              </div>
